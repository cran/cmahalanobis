...
cmanhattan <- function(dataset, formula, plot = TRUE, plot_title = "Manhattan Distance Between Groups") {
  ...
  covariances <- lapply(groups, function(df) {
    cov_matrix <- cov(df)
    n <- nrow(cov_matrix)
    reg <- 0.00001  # Valore di regolarizzazione
...
if (!is.null(distances) && !all(is.na(distances))) {
  knitr::kable(distances, caption = "Manhattan Distances")
} else {
...
if (!is.null(distances) && !all(is.na(distances))) {
  plot_manhattan_distances(distances, "Manhattan Distance Between Groups")
} else {
