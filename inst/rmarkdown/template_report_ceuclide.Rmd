...
pvaluesceucl <- function(dataset, formula, pvalue.method = "chisq", num.permutations = 100, num.bootstraps = 10, plot = TRUE) {
  ...
  if (plot) {
    ...
    print(ggplot2::ggplot(data = p_values_melt, ggplot2::aes(x = Var1, y = Var2, fill = value)) +
            ...
            ggplot2::labs(title = "p_values heatmap", x = "", y = "", fill = "p_valore") +
            ggplot2::theme_minimal() +
...
if (!is.null(distances) && !all(is.na(distances))) {
  knitr::kable(distances, caption = "Euclidean Distances")
} else {
...
if (!is.null(distances) && !all(is.na(distances))) {
  plot_euclidean_distances(distances, "Euclidean Distance Between Groups")
} else {
