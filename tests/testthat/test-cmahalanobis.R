...
#'
#' Calculate the Mahalanobis distance for each group
#'
#' This function takes a list of data frames as input, where each data frame contains the observations of a group, and returns a matrix with the Mahalanobis distances between each pair of groups.
#'
#' @param dataset A list of data frames, where each data frame contains the observations of a group.
#' @return A matrix with the Mahalanobis distances between each pair of groups.
#' @examples
...
cmahalanobis <- function(dataset) {
  ...
  for (i in 1:n) {
    for (j in 1:n) {
      ...
      # Use the correct order of the matrices and transpose the input matrix if needed
      distances[i, j] <- mean(mahalanobis(x = t(dataset[[j]]), center = mean_i, cov = cov_i))
    }
  }
